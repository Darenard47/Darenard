...
// ═══════════════════════════════════════════════════════════════
// API ROUTE (with retry + model fallback)
// ═══════════════════════════════════════════════════════════════
...
async function tryStream(model, systemPrompt, messages) {
...
app.post("/api/chat", async (req, res) => {
  try {
    ...
    for (const model of MODELS) {
      for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        try {
          ...
          stream = await tryStream(model, SYSTEM_PROMPT, fullMessages);
          lastError = null;
          break;
        } catch (err) {
          lastError = err;
          const isOverloaded =
            ...
            err.cause?.message?.includes("Overloaded") ||
            err.status === 529;
          console.error(`${model} attempt ${attempt} failed: ${isOverloaded ? "overloaded" : err.message}`);

          if (!isOverloaded) break; // non-overload error, skip retries
          if (attempt < MAX_RETRIES) await sleep(RETRY_DELAY_MS * attempt);
        }
      }
      if (stream && !lastError) break; // success, stop trying models
    }
    ...
    if (!res.headersSent) {
      res.status(500).json({ error: "Erreur de communication avec Darenard." });
    } else {
      res.write(`data: ${JSON.stringify({ text: "\n\n— Mon écriture m'échappe… réessayez dans un instant." })}\n\n`);
      res.write("data: [DONE]\n\n");
      res.end();
    }
  }
